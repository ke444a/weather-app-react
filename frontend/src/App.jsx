...
const App = () => {
    ...
    useEffect(() => {
        // Adding event listener to get geolocation on page load
    ...
    // Fetching weather data from OpenMeteo API for current location for 5 days
        ...
        return await axios
            .get(url, {
                params: {
                    ...
                    hourly: "temperature_2m,relativehumidity_2m,apparent_temperature,pressure_msl,cloudcover,windspeed_10m,weathercode",
                    windspeed_unit: "ms",
                    start_date: currentIsoDate,
                    end_date: finalIsoDate,
                },
            })
            .then((res) => res.data);
    };

    // Fetching geolocation data (latitude, longitude) from OpenMeteo API based on city name
    const getGeolocationByCity = async () => {
        const url = "https://geocoding-api.open-meteo.com/v1/search";

        return await axios
            .get(url, { params: { name: city, count: 1 } })
            .then((res) => res.data.results[0]);
    };

    // Fetching city name and country code from OpenMeteo API based on geolocation
    const getCityByGeolocation = async () => {
        const url = import.meta.env.VITE_SERVER_URL;

        return await axios
            .get(url, { params: { lat: latitude, lon: longitude } })
    ...
    const getCurrentWeather = (data) => {
        ...
        setCurrentWeather({
            ...
            description: convertWeatherCode(
    ...
    // Processing the fetched forecast data to get the forecast for every 12 hours
    ...
    if (
        weatherResult.isFetching ||
        cityGeoResult.isFetching ||
        cityCoordsResult.isFetching
    ) {
        return (
            <>
    ...
    if (
        ...
        cityGeoResult.isError ||
        cityCoordsResult.isError
    ) {
        return (
            <>
                <Header submitCity={submitCity} />
